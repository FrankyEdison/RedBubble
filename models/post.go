...
type Post struct {
	...
	PostId     int64      `gorm:"type:bigint;unique;index;not null;comment:帖子id" json:"post_id,string"` // 指定在数据库中的字段类型，唯一，添加索引，非空，响应时转成string格式
	UserId     int64      `gorm:"type:bigint;index;not null;comment:用户id" json:"user_id,string"`        // 指定在数据库中的字段类型，添加索引，非空，响应时转成string格式
	...
	CategoryId int64      `gorm:"type:bigint;index;not null;comment:分类id"`                              // 字段类型，添加索引，非空
